...
namespace CityBuilderTests.Buildings
{
    ...
    public class BuildingTilesOnMapLocatorTests
    {
        ...
        {
            var map = new Map(2,2);
            var result = _cut.Locate(map, new SingleTileBuilding(Guid.NewGuid(), angle), new Point(1, 1));
        ...
        {
            var map = new Map(4, 4);
            var result = _cut.Locate(map, new ShortOrthogonalBuilding(Guid.NewGuid(), angle), new Point(1, 1));
